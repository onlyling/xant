...
const DropdownText: React.FC<DropdownTextProps> = ({
  ...
  const textStyleSummary = StyleSheet.flatten<TextStyle>([
    Styles.text,
    {
      color: disabled ? themeVar.dropdown_menu_title_disabled_text_color : active ? activeColor : themeVar.dropdown_menu_title_text_color,
    },
    titleTextStyle,
  ]);
  ...
  const ctxJSX = (
    <>
      ...
      <IconArrowFill size={ICON_SIZE} direction={active ? (direction === 'up' ? 'down' : 'up') : direction} color={active ? activeColor : themeVar.gray_4} />
    </>
  );
